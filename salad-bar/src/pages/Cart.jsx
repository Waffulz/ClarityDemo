  ...
  if (items.length === 0) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-20 text-center">
        ...
        <p className="text-sm text-warm-gray mb-6">Looks like you haven't added any salads yet.</p>
        <Link
          to="/"
          className="inline-block bg-sage text-white px-6 py-3 rounded-xl font-medium hover:bg-sage-dark transition-colors no-underline text-sm"
        >
          Browse Salads
        </Link>
      </div>
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="space-y-4 mb-8">
        {items.map((item) => (
          ...
          >
            {/* Image */}
            <Link to={`/salad/${item.salad.id}`} className="shrink-0">
              <img
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-2">
                <div>
                  <Link to={`/salad/${item.salad.id}`} className="text-sm font-semibold text-charcoal no-underline hover:text-sage transition-colors">
                    {item.salad.name}
                  </Link>
                  {item.extras.dressing && (
                    <p className="text-xs text-warm-gray mt-0.5">Dressing: {item.extras.dressing}</p>
                  )}
              ...
              <div className="flex items-center justify-between mt-3">
                <div className="flex items-center gap-2 bg-cream rounded-lg px-2 py-1">
                  <button
                    ...
                    className="p-0.5 hover:text-sage transition-colors bg-transparent border-none cursor-pointer"
                  >
      ...
      <div className="bg-white rounded-2xl border border-cream-dark p-6">
        ...
        <Link
          to="/checkout"
          className="block w-full bg-sage text-white text-center py-3.5 rounded-xl font-medium hover:bg-sage-dark transition-colors no-underline text-sm"
        >
          Proceed to Checkout
        </Link>
        <Link
          to="/"
          className="block text-center text-sm text-warm-gray hover:text-sage mt-3 transition-colors no-underline"
        >
          Continue Shopping
        </Link>
      </div>
